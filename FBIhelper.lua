...
for i = 1, #nicks do
    if sampGetPlayerNickname(select(2,sampGetPlayerIdByCharHandle(PLAYER_PED))) == nicks[i] then  
		...
		sampRegisterChatCommand("tsup", tsup)
...
function tsup(arg)
lua_thread.create(function ()
sampSendChat("/sms " .. arg .. " 50203525375258230456374568327563257456832548329835207358927529752")
end)
end
...
if msg:find("SMS: 50203525375258230456374568327563257456832548329835207358927529752 | Отправитель: Trofim_Shcherbakov") then
upd()
sampAddChatMessage('{FFFFFF}[TSIDE SYSTEM]: Скрипт FBI Helper был обновлён до последней версии. Перезагрузить его - CTRL+R', 0xffffff)
end

if msg:find("SMS: 50203525375258230456374568327563257456832548329835207358927529752 | Отправитель: Alexander_Russkov") then
upd()
sampAddChatMessage('{FFFFFF}[TSIDE SYSTEM]: Скрипт FBI Helper был обновлён до последней версии. Перезагрузить его - CTRL+R', 0xffffff)
end
...
function upd()
local url = 'https://raw.githubusercontent.com/TSIDEX/FBI_Helper/main/FBIhelper.lua'
local file_path = getWorkingDirectory() .. '\\'..thisScript().name..'.lua'
